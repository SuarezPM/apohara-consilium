...
| CC7.5 | Identify and develop recovery from disruption | LobsterTrap fails-open to preserve availability when unreachable; the 12-vendor ensemble degrades gracefully if individual vendors are unavailable | `packages/backend/lobstertrap_client.py:check_prompt_with_lobstertrap()` (fail-open path), `packages/backend/main.py` (ensemble fallback) | Partial | Document a formal recovery time objective (RTO) and recovery point objective (RPO) even if they are aspirational (e.g., RTO 4 hours, RPO 24 hours). |
...
| CC9.2 | Assess and manage risks associated with vendors | 12-vendor adversarial ensemble design means no single vendor failure compromises the verdict; THIRD_PARTY_NOTICES.md inventories all dependencies | `packages/backend/main.py` (ensemble), `THIRD_PARTY_NOTICES.md`, `LICENSE` | Partial | Write a vendor risk register. For each of the 12 model vendors (Google Gemini, OpenRouter vendors, etc.), document: what data is sent, under what terms, and what the fallback is if that vendor is unavailable or breached. |
