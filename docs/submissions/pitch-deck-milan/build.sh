#!/usr/bin/env bash
# Build the Apohara CONSILIUM Milan pitch deck PDF from the HTML source.
...
#
# Usage:   bash build.sh
# Output:  apohara-consilium-milan-pitch.pdf next to index.html
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="${HERE}/index.html"
OUT="${HERE}/apohara-consilium-milan-pitch.pdf"
